...
function useAsyncData<T>(
  ...
  const [state, setState] = useState<{ data: T | null; loading: boolean; error: string | null }>({
    data: null,
    loading: options.immediate !== false, // 修复初始loading状态
    error: null,
  });
...
export function useStories(filters?: StoryFilters, options?: UseAsyncDataOptions) {
  return useAsyncData<PaginatedResponse<StoryData>>(
    () => chainDataService.getStories(filters),
    { 
...
export function useStoryById(id: string, options?: UseAsyncDataOptions) {
  return useAsyncData<StoryData | null>(
    () => chainDataService.getStoryById(id),
    { ...options, dependencies: [id, ...(options?.dependencies || [])] }
  );
}

export function useStoriesByAuthor(author: string, pagination?: { page?: number; limit?: number }, options?: UseAsyncDataOptions) {
  return useAsyncData<PaginatedResponse<StoryData>>(
    () => chainDataService.getStoriesByAuthor(author, pagination),
    { ...options, dependencies: [author, JSON.stringify(pagination), ...(options?.dependencies || [])] }
  );
}
...
export function useChapters(filters?: ChapterFilters, options?: UseAsyncDataOptions) {
  return useAsyncData<PaginatedResponse<ChapterData>>(
    () => chainDataService.getChapters(filters),
    { ...options, dependencies: [JSON.stringify(filters), ...(options?.dependencies || [])] }
  );
}

export function useChapterById(id: string, options?: UseAsyncDataOptions) {
  return useAsyncData<ChapterData | null>(
    () => chainDataService.getChapterById(id),
    { ...options, dependencies: [id, ...(options?.dependencies || [])] }
  );
}

export function useChaptersByStory(storyId: string, pagination?: { page?: number; limit?: number }, options?: UseAsyncDataOptions) {
  return useAsyncData<PaginatedResponse<ChapterData>>(
    () => chainDataService.getChaptersByStory(storyId, pagination),
    { ...options, dependencies: [storyId, JSON.stringify(pagination), ...(options?.dependencies || [])] }
  );
}

export function useChaptersByParent(parentId: string, pagination?: { page?: number; limit?: number }, options?: UseAsyncDataOptions) {
  return useAsyncData<PaginatedResponse<ChapterData>>(
...
export function useMonitorStatus(options?: UseAsyncDataOptions) {
  return useAsyncData<any>(
    () => chainDataService.getMonitorStatus(),

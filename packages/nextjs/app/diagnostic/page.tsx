...
export default function DiagnosticPage() {
  ...
  const tests = [
    ...
    },
    {
      ...
      key: "checkTables",
      action: () =>
        runTest("checkTables", "/api/debug/init-db", {
          method: "POST",
    ...
    },
    {
      ...
      key: "initTables",
      action: () =>
        runTest("initTables", "/api/debug/init-db", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    },
    {
      ...
      key: "testComment",
      action: () =>
        runTest("testComment", "/api/debug/comments", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    },
    {
      ...
      key: "allComments",
      action: () =>
        runTest("allComments", "/api/debug/comments", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    },
    {
      ...
      key: "tokenComments",
      action: () =>
        runTest("tokenComments", "/api/debug/comments", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    },
    {
      ...
      key: "clearComments",
      action: () =>
        runTest("clearComments", "/api/debug/init-db", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        {tests.map(test => (
          <button
            ...
            className={`btn ${loading === test.key ? "loading" : ""} ${
              results[test.key]?.success === true
                ? "btn-success"
                : results[test.key]?.success === false
                  ? "btn-error"
                  : "btn-primary"
            }`}
          >
      ...
      <div className="mt-8 p-6 bg-base-200 rounded-lg">
        ...
        <ol className="list-decimal list-inside space-y-2">
          <li>
            <strong>检查数据库表</strong> - 验证comments表是否存在
          </li>
          <li>
            <strong>初始化数据库表</strong> - 如果表不存在，创建所需的表和索引
          </li>
          <li>
            <strong>数据库连接</strong> - 测试数据库连接和基本操作
          </li>
          <li>
            <strong>监控状态</strong> - 检查链上数据监控是否正在运行
          </li>
          <li>
            <strong>启动监控</strong> - 如果监控未运行，尝试启动它
          </li>
          <li>
            <strong>插入测试评论</strong> - 手动插入一条测试评论到数据库
          </li>
          <li>
            <strong>查询评论</strong> - 验证评论是否成功插入
          </li>
          <li>
            <strong>手动同步事件</strong> - 强制同步链上事件
          </li>
        </ol>
      </div>

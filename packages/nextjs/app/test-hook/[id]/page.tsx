...
export default function TestHookPage() {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-4">
        <strong>故事数据:</strong>
        <pre className="bg-gray-100 p-2 text-sm overflow-auto">{JSON.stringify(story, null, 2)}</pre>
      </div>
      <div>
        <strong>章节数据:</strong>
        <pre className="bg-gray-100 p-2 text-sm overflow-auto">{JSON.stringify(chapters, null, 2)}</pre>
      </div>
    </div>
  );
}
